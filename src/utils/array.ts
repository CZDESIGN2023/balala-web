...
// 定义一个泛型函数，合并两个不同类型的数组
export function mergeObjectsByKey<T1, T2, K extends keyof T1 & keyof T2, R>(
  arr1: T1[],
  arr2: T2[],
  key: K,
): R[] {
  ...
  arr2.forEach((item) => {
    ...
    if (mergedByKey[keyValue]) {
      // 如果存在相同的 key 值，则合并对象
      mergedByKey[keyValue] = { ...mergedByKey[keyValue], ...item } as R
    }
    else {
      // 如果不存在相同的 key 值，则直接添加到 mergedByKey 对象中
      mergedByKey[keyValue] = { ...item } as unknown as R
    }
  })
