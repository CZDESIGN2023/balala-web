  ...
  >
    <template #content>
      <div v-if="orderRt.optionSelect.length" class="order-ul">
        <draggable
          v-model="orderRt.optionSelect"
          :force-fallback="true"
          :fallback-on-body="true"
          chosen-class="search-group-chosen"
        ...
        >
          <template #item="{ element, index }">
            <div
              class="order-li flex"
              :class="{
                'del-hover': element.hover,
              }"
            >
              <!-- 拖拽图标 -->
              <div class="drag-icon-container flex-row-center mr12">
                <svg-icon
              ...
              >
                <template #option="{ label, icon }">
                  <div class="flex-row-start gap8 text14">
                    <svg-icon :name="icon" size="16" color="#666666" />
                    {{ label }}
                  </div>
                </template>
              </BBaseSelectTool>
