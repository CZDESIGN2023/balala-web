...
const emits = defineEmits(['onChange', 'onClose', 'onShow', 'onRecovery', 'onChangeSameDate'])
...
function isShowTips() {
  if (contentRef.value) {
    setTimeout(() => {
      if (contentRef.value?.querySelector('.ant-input')) {
...
function handleConfirm(emitName: any) {
  ...
  if (timeRange.value.length === 0) {
    ...
    timeValue.value = `${dayjs().format(DateFormat.DATE)} ~ ${dayjs().format(DateFormat.DATE)}`
  }
  selectTime.value = {
...
function dropdown(visible: boolean = true) {
  popupVisible.value = visible
