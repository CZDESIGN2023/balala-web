...
const introTrendsHeight = ref(0)
const isIncreaseEditor = ref<boolean>(false)
const increaseBtnShow = ref<boolean>(false)
...
const calcReadStyle = computed(() => {
  return {
    minHeight: `${props.mode === 'comment' ? 32 : introTrendsHeight.value ? introTrendsHeight.value : props.minHeight}px`,
...
// 外部点击展开评论富文本
function openCommentEditor(item: CommentItem) {
  readonlyMode.value = false
  insertReplyContent(item)
...
watch(
  ...
  () => {
    requestAnimationFrame(() => {
      ...
      const contentHeight = document.querySelector('.read-content')?.clientHeight || 0
      if (contentHeight + 26 > props.minHeight) {
        increaseBtnShow.value = true
      }
      else {
        introTrendsHeight.value = props.minHeight
        increaseBtnShow.value = false
        isIncreaseEditor.value = false
      }
    })
  },
  {
...
// 增加可视区域高度 * 3
function increaseConentHeight() {
  const newHeight = isIncreaseEditor.value ? props.minHeight : 468
  introTrendsHeight.value = newHeight

  isIncreaseEditor.value = !isIncreaseEditor.value
}
...
<template>
  <div ref="editorboxRef" :class="{ 'b-editor-view': view }" class="b-editor">
    ...
    <el-scrollbar
      ...
      :max-height="`${props.mode === 'introMode' ? introTrendsHeight || 128 : introTrendsHeight || props.maxHeight}px`"
      :class="[calcEditorValue, calcReadClass, props.type]"
      @click="changeEditMode"
    >
      <ElTooltip v-if="props.mode === 'introMode' && increaseBtnShow" :content="isIncreaseEditor ? '收起' : '展开'" placement="top" :hide-after="0">
        <div class="read-content-increase" :class="{ active: isIncreaseEditor }" @click.stop="increaseConentHeight">
          <SvgIcon class="arrow-icon" name="editor_arrow" color="#666" size="16" />
        </div>
      </ElTooltip>
      <SvgIcon v-if="mode === 'comment' && commentStyle === 'short'" name="comment-icon" color="#666" size="16" />
