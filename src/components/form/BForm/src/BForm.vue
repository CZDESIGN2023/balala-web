...
function onChangeDate(item: any, val: any) {
  onChange(item.field, val)
  onUpdateModelValue(item, val)
  ...
  >
    ...
    >
      <div v-for="d in itemsArray" :key="d.field" :style="{ width: d.width }">
        ...
        <div v-else-if="d.type === 'line'" class="line mb24" />
        ...
        >
          ...
          <b-input-date
            ...
            v-bind="d"
            width="100%"
            :info="$getValueByPath(d.field)"
            @on-change="(planTime:any) => onChangeDate(d, planTime)"
          />
