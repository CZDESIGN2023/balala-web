...
<template>
  ...
  >
    <template #suffixIcon>
      <SvgIcon name="select-arrow" size="16" color="#333333" />
    </template>
    ...
    <template #tagRender="{ option }">
      <a-tag v-if="option && option.value" class="filter-select-tag flex-row-start" :closable="true" @click="state.open = true" @close="onClose(option?.value)">
        <p class="text13 title-color ss-line-1 maxwi65 mr1">
          {{ option?.label }}
        </p>
        <template #closeIcon>
          <SvgIcon
