...
const placeholder = ref<string>('请选择标签，或输入内容回车后新建标签')
...
function selectFocus() {
  ...
  if (!templateList.value.length)
    placeholder.value = '请输入内容，回车后新建标签(2 ~ 12个字符)'
  else
    placeholder.value = '请选择标签，或输入内容回车后新建标签(2 ~ 12个字符)'
}
// 失去焦点
function selectBlur() {
  handleSearch('') // 选中标签, 清空状态
  visibleDropDown.value = false
  placeholder.value = '请选择标签，或输入内容回车后新建标签'
              ...
              <b style="font-weight: normal; text-align: center;">
                <span>
                  <span v-if="enableCreate">
                    未找到与<span class="primary-color" style="word-break: break-all;">“{{ searchText }}”</span>相关的标签，回车后可新建标签</span>
