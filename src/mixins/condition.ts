...
export function formatConditionData(data: ConditionGroup): QueryConditionGroup {
  ...
  filterData.forEach((item) => {
    ...
    if (['plan_time', 'created_at', 'plan_complete_at'].includes(oneSelectVal as string)) {
...
// 已完成任务
export function getCompletedTasks(isProject: boolean) {
  const { twoOptions: workTwoOptions } = getFilterOptions('work_item_status')

  const workCondition = createCondition(isProject, 'work_item_status', workTwoOptions, 'IN', ['1'])
  return {
    conjunction: 'AND',
...
export function parseToUIData(data: QueryConditionGroup) {
  ...
  if (data.conditions) {
    data.conditions.map(async (item) => {
      ...
      const thirdValue
        = (['plan_time', 'created_at', 'plan_complete_at'].includes(item.field))
...
async function getFilterName(obj: any) {
  ...
  else if (['plan_time', 'created_at', 'plan_complete_at'].includes(oneSelectVal) && obj?.thirdSelectVal?.start && obj?.thirdSelectVal?.complete) {
...
export function generateUniqueConditionName(conditions: any[]): string {
  const pattern = /条件(\d+)/
...
export function getJumpCondition(options: any) {
  const { type, idField, id, startTime, endTime } = options
  // 任务创建时间
  ...
  const participatorsCondition = {
    field: idField,
    operator: `${idField === 'directors' ? 'IN' : 'EQ'}`,
    values: [String(id)],
    spaceId: '0',
  }
  ...
  if (type === 'expired') {
    conditions.push(planTimeCondition)
  }
  conditions.push(participatorsCondition)
  conditions.push(createTimeCondition)
  return {
    conditions,
    conjunction: 'AND',
    conditionGroup,
