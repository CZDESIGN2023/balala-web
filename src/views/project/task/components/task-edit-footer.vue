...
<template>
  <div
    class="drawer-footer flex-row-between pr24 pl24 gap12"
    ...
    <div class="commnet-box">
      <div v-if="taskPermissions.allowComment" class="default-mode flex-row-start">
        <b-editor
          ...
          :read-content="tabKey === 'comment' ? '请输入评论（Enter 换行，⌘ + Enter / Ctrl+Enter 发送）' : '评论...'"
          :resize="false"
          :min-height="140"
          :max-height="436"
          :mode-key="`${state.workItemId}comment`"
          :comment-style="tabKey === 'comment' ? 'long' : 'short'"
    ...
    <div class="btn-box flex-row-center gap12">
      <div v-for="item in state.footerBtns" :key="item.handle">
        ...
        >
          ...
          <a-button class="btn cancel-btn" size="default" plain>
            关闭
          </a-button>
        </b-table-popover>
        <a-button
