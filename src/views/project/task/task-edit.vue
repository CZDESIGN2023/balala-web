...
function replayComment(item: CommentItem) {
...
function addCommentSuccess() {
  nextTick(() => {
    const scrollHeight = state.commentSort === 'ASC' ? scrollFun.value?.wrapRef.scrollHeight : 0
    scrollFun.value!.setScrollTop(scrollHeight)
  })
}
...
function handleScroll(e: any) {
  if (e.scrollTop > 600 && state.tabKey === 'comment')
  ...
  if (
    ...
    >= scrollFun.value.wrapRef.scrollHeight - 10
  ) {
...
function commentScroll(type: string) {
  if (type === 'top')
    scrollFun.value!.setScrollTop(0)
  if (type === 'bottom')
    scrollFun.value!.setScrollTop(scrollFun.value?.wrapRef.scrollHeight)
}
  ...
  >
    ...
    <template #title>
      <div class="drawer-header flex-row-between">
        <BSkeleton class="task-header-info-wrap" :loading="isShowHeaderSkeleton" height-class="h32">
          <template v-if="!state.taskIsError">
            <TaskHeaderInfo ref="taskHeaderInfoRef" @on-update-form-item="changeFormItem" />
          </template>
          <template v-else>
            <p class="text18 pfm icon-color">
              {{ state.taskErrorText }}
            </p>
          </template>
        </BSkeleton>
        <div class="drawer-header-tool flex-row-end ml44">
          <TaskHeaderTool
          ...
          />
          <div class="close flex-row-center pointer ml12 flex-shrink-0" @click="closeTaskDrawer">
