...
const itemsBefore: { [key: string]: ItemProps } = {
  ...
  priority: {
    ...
    field: 'priority',
    width: '200px',
    labelCol: { style: { 'width': '70px', 'flex-shrink': 0 } },
    rules: [
      {
        required: true,
        trigger: ['change'],
...
const itemsAfter: { [key: string]: ItemProps } = {
  ...
  tag: {
    ...
    field: 'tagIds',
    width: '100%',
    placeholder: '请选择标签，或输入内容回车后新建标签',
    spaceId: '',
  },
...
export async function getFormItems(workflow: WorkFlowTemplateData, taskDetail?: WorkDetailData, taskPermissions?: TaskPermission) {
  ...
  const newRoleOwners = roleOwners.map((item, index) => {
    ...
    return Object.assign(item, {
      ...
      type: 'personSelect',
      field: item.roleKey,
      width: index === 0 ? 'calc(100% - 16px - 200px)' : '100%',
  ...
  newRoleOwners.forEach((item, index: number) => {
    ...
    if (index === 0)
      nodesObj.$creator = { label: '创建人', type: 'personSelect', field: '$creator', width: '200px', labelCol: { style: { width: '70px' } }, disabled: true }
  })
