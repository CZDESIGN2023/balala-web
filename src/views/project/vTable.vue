  ...
  >
    ...
    >
      <template #bottom>
        <div
          v-if="!loading"
          class="table-footer flex-row-between pl16 pr16 text13 main-color"
        >
      ...
      <!-- 序号和多选框 内容 -->
      <template #checkbox_default="{ row, _rowIndex, level }">
        <!-- 分组行 -->
        <div
          v-if="row.isGroup"
          class="flex-row-start module-name flex-inline flex-row-start full-100 pointer"
        >
          <div v-if="level !== 0" class="group-child flex-row-center mr8 ml14">
            <svg-icon name="group-child" size="16" color="#999999" />
          </div>
          <div
            class="x-expand-icon flex-row-center mr6 "
            :class="{ rotate: row.isExpanded === 'fold', ml14: level === 0 }"
          >
            <svg-icon name="table-group-bottom" size="16" color="#999999" />
          </div>
          <p :style="{ background: getGroupColor(row) }" class="mr8 flex-row-center pfm">
        ...
        <!-- 可勾选行 -->
        <div
          v-else-if="!row.isGroup && row.pid === '0'"
          class="checkbox-box flex-row-center"
        ...
        <template v-if="!row.isGroup">
          <div
            class="followed pointer"
            @click="onFollow(row)"
          >
            <img v-if="row.followed" src="@/assets/icon/followed.png" @mouseover="(e) => tooltipMouseover(e, '取消关注')">
            <img v-else class="follow" src="@/assets/icon/follow.svg" @mouseover="(e) => tooltipMouseover(e, '关注任务')">
          </div>
          <div class="copy-workitem-id pointer" @click="onCopyWorkItemId(row)" @mouseover="(e) => tooltipMouseover(e, '复制 ID')">
            <img class="follow" src="@/assets/svg/id.svg">
            <img class="follow" src="@/assets/svg/id-dark.svg">
          </div>
        </template>
      </template>
      ...
      <!-- 任务名称 -->
      <template #displayName_default="{ row, _rowIndex, islast }">
        <BSkeleton :loading="row.loading">
          <div class="displayname-box flex-row-start">
            ...
            >
              ...
              />
              <span
                class="ss-line-1 icon-color pointer ml4"
                @click="openDrawer(row, _rowIndex, 'base')"
              >
                <b-ellipsis
                  :content="row.workItemName"
                  @mouse-over-back="(e: MouseEvent) => tooltipMouseover(e, row.workItemName)"
                />
              </span>
      ...
      <template #director_default="{ row }">
        ...
        >
          <div
            class="director-module flex-row-start pointer"
            :class="{
              'not-rate-allow': !row.perm.allowEdit,
            }"
      ...
      <template #time_default="{ row }">
        <BSkeleton :loading="row.loading">
            ...
            @click="
              (e) => {
                if (row.perm.allowEdit) {
                  bTablePopupRef.pop(
                    e,
                    { type: 'planDate', clickOutside: false, border: false },
                    { row, maxlength: 3, plain: true },
                  )
                }
              }
            "
          >
            <b-ellipsis
              :content="row.formatTime"
              @mouse-over-back="(e: MouseEvent) => tooltipMouseover(e, row.formatTime)"
            />
          </p>
        </BSkeleton>
      </template>
