...
const { emitUpdate } = useEmitter<string>(EmitEvent.UPDATE_MODULE_LIST)
...
const moduleData = ref<ModuleItem[]>([]) // 模块列表数据
...
function handleScroll(e: { scrollLeft: number, scrollTop: number }) {
  introModuleGrid.value?.scrollTo(0, e.scrollTop)
}
...
async function getList(isShowLoading: boolean = false) {
  try {
    ...
    const list = await getModuleList(id || '', true)
    emitUpdate('updateModuleList')
    setTimeout(() => {
      emit('handleLoading', false)
      moduleData.value = deepCopy(list || [])
      const options: any = []
...
async function handleInsert() {
  ...
  moduleData.value.unshift(recordRow as any)
  const $grid = introModuleGrid.value
  if ($grid) {
  ...
  <div class="table-wrap">
    ...
    >
      ...
      <template #name_default="{ row, _rowIndex }">
        <div v-if="!row.contenteditable" class="text14 icon-color flex-row-between name-box pr12">
      ...
      <template #priority_default="{ row }">
        <div class="priority flex-row-between">
          <div
            v-for="item in row.stats.priorityInfos"
            :key="item.priority"
            class="pointer"
      ...
      <template #tool_default="{ row }">
        <div
          v-if="
            ((moduleData[0] as any)?.contenteditable
              ? moduleData.length > 2
    ...
    <el-scrollbar
      ...
      @scroll="handleScroll"
    >
      <div class="w6" :style="{ height: `${moduleData.length * 48}px` }" />
    </el-scrollbar>
  </div>
  <!-- 移除模块确认弹框 -->
  <b-dialog
