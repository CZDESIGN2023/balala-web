...
import { getProjectOptions } from '../../config/user'
...
async function getMemberlist(isShowLoading: boolean = false) {
  try {
    ...
    const list = await getUserList(id || '', true)
    emitUpdate('updateUserList')
    const memberList = await projectStore.getProjectMemberList(id || '', '', [], false)
    setTimeout(() => {
      emits('handleLoading', false)
      memberData.value = deepCopy(list?.filter((find: any) => find.roleId < 98) || [])
