...
async function getInfo(isShowLoading: boolean = false) {
  try {
    ...
    await initProjectInfo(router.currentRoute.value.params.id as string)
    setTimeout(() => {
      emits('handleLoading', false)
      loadingEnd.value = true
      spaceInfo.value = projectInfo.value
      checkedNotify.value = Number(projectInfo.value.notify)
      checkedComment.value = Number(projectInfo.value.commentDeletable)
      spaceName.value = projectInfo.value.spaceName as string
      spaceStore.setUserStatus(Number(projectInfo.value.roleId))
      spaceWorkingDay.value = JSON.parse(projectInfo.value.workingDay as string) || []
    }, 400)
  }
  catch (err) {
    loadingEnd.value = true
...
// 富文本编辑器失焦 修改项目描述
async function editorBlur(describe: string) {
  // 修改信息
  ...
  try {
    if (spaceInfo.value) {
      ...
      message.success('项目描述修改成功', 3)
      spaceInfo.value.describe = describe
    }
  }
  catch (error) {
    console.log('修改项目描述:', error)
  }
}

// 项目名称失去焦点
function spaceNameBlur() {
  const trimmedSpaceName = spaceName.value.trim()
  const nameLength = getTextLength(trimmedSpaceName)
  const isValidLength = nameLength >= 2 && nameLength <= 20
  const isNameUnchanged = trimmedSpaceName === spaceInfo.value?.spaceName
  ...
  if (!isValidLength) {
    const errorMessage = nameLength ? '请输入 2 ~ 20个字符' : '项目名称不能为空'
    message.error(errorMessage, 2)
    spaceName.value = spaceInfo.value?.spaceName || ''
    return
  }
...
async function editSpaceName() {
  try {
    if (spaceInfo.value) {
      ...
      spaceInfo.value.spaceName = spaceName.value.trim()
      await projectStore.getList()
    }
  }
  catch (error) {
    console.log('修改项目名称:', error)
...
<template>
  <div v-if="loadingEnd" class="project-space-basic pt8">
    ...
    <div class="cell mt32">
      ...
      <div class="h32 flex-row-between text14 minor-color">
        {{
          checkedNotify
    ...
    <div class="cell mt32">
      ...
      <div class="weekday-box mt8 h56 flex-row-between">
        <BasicWeekday
