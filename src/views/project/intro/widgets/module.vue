...
const addToolRef = ref() // 添加工具
const moduleToolHeadRef = ref() // 模块头部工具
const inputRef = ref() // 编辑输入框
const bTableRef = ref() // 表格
...
const tableData = ref<ModuleItem[]>([]) // 表格数据
...
// 编辑模块 监听输入框失去焦点
function inputBlur(row: ModuleItem) {
  const length = getTextLength(row.workObjectName.trim())
  if (row.workObjectName.trim() === originalName.value) {
    return
  }
  if (length < 2 || length > 14) {
    message.error('请输入有效格式(2 ~ 14个字符)', 3)
    row.workObjectName = originalName.value
    return
  }
  editModuleName(row)
}
...
// 编辑模块
async function editModuleName(row: ModuleItem) {
  try {
...
// 日期变化重新请求列表数据
async function dateChange(dateRange: DateRange) {
  const spaceId = router.currentRoute.value.params.id as string
  const { startTime, endTime } = dateRange
  dataRange.value = dateRange

  const data = await getModuleList(spaceId, true, startTime, endTime) as ModuleItem[]
...
// 用于获取 HoverData 的选项
function getOptions(column: any, row: ModuleItem) {
  if (column.prop === 'stats.processing') {
    ...
    return [
      { color: '', label: '模块进度', value: `${row.stats.completeRate}%` },
...
// 更新模块列表
useEmitter(EmitEvent.UPDATE_MODULE_LIST, () => {
  moduleToolHeadRef.value.dateChange()
})
...
<template>
  <introItem v-model="isFullScreen" class="w-full">
    <div :class="{ 'full-screen': isFullScreen }" class="intro-module">
      ...
      >
        <template #leftTool>
          <addToolPopover v-if="checkPerm(Perm.CreateSpaceWorkObject)" ref="addToolRef" title="模块" placeholder="请输入模块名" @on-add="createModule" />
        </template>
      </intro-module-head>
      <div class="intro-module-grid">
        <div class="intro-module-table">
          ...
          >
            ...
            <!-- 模块名称 -->
            <template #workObjectName="{ row, $index }">
              <div class="name-box text14 flex-row-between">
                ...
                <div
                  ...
                  @click="tableColumnEdit('workObjectName', $index)"
                >
            ...
            <template #stats-expired="{ row }">
              <p class="num-box" @click="linkToTable({ type: 'expired', idField: 'work_object_id', id: row.id })">
                {{ row.stats.expired }}
              </p>
            </template>
            <!-- 进度 -->
            ...
            <template #stats-processing="{ row }">
              <p class="num-box" @click="linkToTable({ type: 'processing', idField: 'work_object_id', id: row.id })">
                {{ row.stats.processing }}
              </p>
            </template>
            <!-- 图表 -->
            <template #chart="{ row }">
              <barChart :title="row.workObjectName" :stats="row.stats" />
            </template>
            <template #hover-data="{ column, row }">
              ...
              <HoverData
                ...
                :options="getOptions(column, row)"
                :title="row.workObjectName"
                :min-width="204"
              />
            </template>
            <template #edit="{ column, row, id }">
              <BInput2
                v-if="column.prop === 'workObjectName'"
                ref="inputRef"
                ...
                plain
                style="padding: 8px;"
                @focus="originalName = row.workObjectName"
                @press-enter="() => {
                  inputRef?.blur()
