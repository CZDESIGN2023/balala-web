...
function selectTag(item: QueryCondition, isChange: boolean) {
  ...
  if (isChange && activeName.value !== 'follow') {
    followData.value = []
...
onMounted(() => {
  ...
  activeName.value = window.localStorage.getItem(`projectTab${props.id}`) ?? 'table'
  updateIntro()
})
...
onBeforeRouteUpdate((to: RouteLocationNormalized) => {
  if (to?.query?.type) {
    activeName.value = to.query.type.toString()
...
<template>
  <div class="project-wrap">
    <!-- 项目固定头部 -->
    <div class="project-header pt24 mb20">
      <div class="top h40 flex-row-between">
        <div id="projectName" class="name flex-row-start skeleton-style" @click="toggleSpace">
          <a-skeleton :paragraph="false" :loading="isSkeletonLoading" active>
            <span class="mr8 ss-line-1 animate__animated animate__fadeIn pfm">
              {{ projectInfo.spaceName }}
            </span>
            <svg-icon name="project-set" color="#333333" size="24" />
          </a-skeleton>
        </div>
        <div class="tags flex-row-start">
          <b-tag
            ref="tagComponent"
            :list="tagList"
          ...
          <a-button
            ...
            class="b-gradient-btn ml40"
            type="primary"
            @click="openAddTaskDrawer"
          >
            <template #icon>
              <svg-icon name="add-white" size="14" color="#fff" />
            </template>
            新建
          </a-button>
        </div>
      </div>
      <div class="tab-header mt24 flex-row-between">
        <div class="left">
          <div
            v-show="activeName === 'table' || activeName === 'follow'"
        ...
        <div class="right flex-row-end">
          <div
            v-for="item in tabNames"
            :key="item.key"
            class="tab-pane flex-row-center gap4 w72 h32 ml8 br6"
            :class="{ active: item.key === activeName }"
            @mousedown.left="changeTab(item.key)"
          >
            <img v-if="item.key === 'intro'" class="w16" src="@/assets/icon/intro.png" alt="">
            <img v-if="item.key === 'table'" class="w16" src="@/assets/icon/table.png" alt="">
            <img
              v-if="item.key === 'follow'"
              class="w16"
              src="@/assets/icon/follow-task.png"
              alt=""
            >
            <p class="text14">
              {{ item.name }}
    ...
    <div class="project-bodyer">
      ...
      <v-table
        v-else-if="activeName === 'table' || activeName === 'follow'"
        ref="projectTableRef"
    ...
    <!-- 项目空间弹框 -->
    <ProjectSpace v-if="spaceVisible" ref="projectSpaceRef" :dialog-visible="spaceVisible" :space-id="Number(props.id)" @on-close="toggleSpace" />
    <!-- 离开项目弹框 -->
