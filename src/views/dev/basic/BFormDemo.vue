    ...
    <example>
      ...
      <b-form
        ...
        :items="[
          ...
          { label: '性别', field: 'sex', type: 'select', width: 'calc(100% - 320px)', options: [{ label: '男', value: 1 }, { label: '女', value: 0 }], rules: [{ required: true, message: '' }] },
          { label: '班级', field: 'class', type: 'select-sync', width: '300px', tips: '测试', queryOptions: queryOptionsPromise },
          { label: '启用', field: 'enable', type: 'switch', width: '100%', tips: '测试2', style: 'justify-content: center;', rules: [{ required: true, message: '' }] },
          { label: '深度对象', field: 'deep.value', type: 'input', width: '50%', placeholder: '支持对象路径作为field，如: deep.value', rules: [{ required: true, message: '必填字段' }] },
          { type: 'placeholder', width: 'calc(100% - 50%)' },
          { label: '深度对象2', field: 'deep.value', type: 'input', width: '300px', placeholder: '支持对象路径作为field，如: deep.value', rules: [{ required: true, message: '必填字段' }] },
          { field: 'names', type: 'personSelect', label: '禁用项', disabled: true, width: '800px' },
          { field: '进度', type: 'input-number', label: '进度', suffix: '%', inputSuffix: '%' },
