...
const originalTitle = ref(document.title) // 存储原始标题
...
function htmlToText(html: string): string {
  // 将 <br> 标签替换为换行符
  let textWithBreaks = html.replace(/<br\s*\/?>/g, '\n')

  // 移除所有 HTML 标签
  textWithBreaks = textWithBreaks.replace(/<[^>]*>/g, '')

  // 移除每行前后的空白字符
  textWithBreaks = textWithBreaks.split('\n').map(line => line.trim()).join('\n')

  // 移除连续的换行符
  textWithBreaks = textWithBreaks.replace(/\n+/g, '\n')

  // 移除首尾的换行符
  textWithBreaks = textWithBreaks.trim()
    ...
    if (notificationInfo.isPopup) {
      ...
      Notification.requestPermission().then((permission) => {
        if (permission === 'granted') {
          const notification = new Notification('收到 待办/通知 消息', {
            dir: 'auto',
            lang: '',
            body: htmlToText(notificationInfo.description),
            tag: '',
            icon: siteLogo.value,
            data: '',
          })
          notification.onclick = () => {
            ...
            const regex = /#([^#]*)$/
            const redirectLink = notificationInfo.data.redirectLink.match(regex)
            const link = redirectLink && redirectLink[1]

            // if (domainRegex.test(notificationInfo.data.redirectLink)) {
            //   redirectLink = notificationInfo.data.redirectLink.replace(domainRegex, '').replace('/#', '')
            // }
            // else {
            //   redirectLink = notificationInfo.data.redirectLink.replace('/#', '')
            // }
...
<template>
  <div class="app-main" :class="isCollapse ? 'app-main-collapse' : 'app-main-expand'">
    <router-view />
...
.app-main-collapse {
  width: calc(100% - $sidebar-width-is-collapse);
  // transform: translateX($sidebar-width-is-collapse);
  margin-left: $sidebar-width-is-collapse;
}
</style>
